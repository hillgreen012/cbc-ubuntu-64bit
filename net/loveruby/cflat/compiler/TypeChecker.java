...
class TypeChecker extends Visitor {
	...
	public Void visit(OpAssignNode node) {
		...
		if (!opType.isCompatible(l) && !isSafeIntegerCast(node.rhs(), opType)) {
			warn(node, "incompatible implicit cast from " + opType + " to " + l);
		}
	 ...
	 *
	 * * integer + integer * pointer + integer * integer + pointer * integer -
	 * integer * pointer - integer * pointer - pointer
	 */
	 ...
	 * For EXPR(ARG), checks:
	 *
	 * * The number of argument matches function prototype. * ARG matches function
	 * prototype. * ARG is neither a struct nor an union.
	 */
	...
	private ExprNode castOptionalArg(ExprNode arg) {
		...
		Type t = arg.type().isSigned() ? typeTable.signedStackType() : typeTable.unsignedStackType();
