...
public class CodeGenerator implements net.loveruby.cflat.sysdep.CodeGenerator, IRVisitor<Void, Void>, ELFConstants {
	// #@@range/ctor{
	...
	static private final String PICThunkSectionFlags = SectionFlag_allocatable + SectionFlag_executable
			+ SectionFlag_sectiongroup;

	/**
	 * Output PIC thunk. ELF section declaration format is:
	 *
	 * .section NAME, FLAGS, TYPE, flag_arguments
	 *
	 * FLAGS, TYPE, flag_arguments are optional. For "M" flag (a member of a section
	 * group), following format is used:
	 *
	...
	private void PICThunk(AssemblyCode file, Register reg) {
		...
		file._section(".text" + "." + sym.toSource(), "\"" + PICThunkSectionFlags + "\"", SectionType_bits, // This
															// section
															// contains
															// data
	 ...
	 *
	 * ======================= esp #3 (stack top just before function call) next arg
	 * 1 --------------------- next arg 2 --------------------- next arg 3
	 * --------------------- esp #2 (stack top after alloca call) alloca area
	 * --------------------- esp #1 (stack top just after prelude) temporary
	 * variables... --------------------- -16(%ebp) lvar 3 ---------------------
	 * -12(%ebp) lvar 2 --------------------- -8(%ebp) lvar 1 ---------------------
	 * -4(%ebp) callee-saved register ======================= 0(%ebp) saved ebp
	 * --------------------- 4(%ebp) return address --------------------- 8(%ebp)
	 * arg 1 --------------------- 12(%ebp) arg 2 --------------------- 16(%ebp) arg
	 * 3 ... ... ======================= stack bottom
	 */
	...
	static final RegisterClass[] CALLEE_SAVE_REGISTERS = { RegisterClass.BX, RegisterClass.BP, RegisterClass.SI,
			RegisterClass.DI };
	...
	private void locateParameters(List<CflatParameter> params) {
		long numWords = PARAM_START_WORD;
		for (CflatParameter var : params) {
	...
	/**
	 * Allocates addresses of local variables, but offset is still not determined,
