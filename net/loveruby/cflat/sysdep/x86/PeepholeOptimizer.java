...
public class PeepholeOptimizer {
	...
	private void loadDefaultFilters() {
		...
		// mov
		set.add(new SingleInsnFilter(new InsnPattern("mov", imm(0), reg()), new InsnTransform() {
		...
		// add
		set.add(new SingleInsnFilter(new InsnPattern("add", imm(-1), reg()), new InsnTransform() {
			public Instruction apply(Instruction insn) {
		...
		// sub
		set.add(new SingleInsnFilter(new InsnPattern("sub", imm(-1), reg()), new InsnTransform() {
			public Instruction apply(Instruction insn) {
				return insn.build("inc", insn.operand2());
			}
		}));
		...
		// imul
		set.add(new SingleInsnFilter(new InsnPattern("imul", imm(0), reg()), new InsnTransform() {
			public Instruction apply(Instruction insn) {
				return insn.build("xor", insn.operand2(), insn.operand2());
			}
		}));
		set.add(new SingleInsnFilter(new InsnPattern("imul", imm(1), reg()), null));
		set.add(new SingleInsnFilter(new InsnPattern("imul", imm(2), reg()), new InsnTransform() {
			public Instruction apply(Instruction insn) {
				return insn.build("sal", imm(1), insn.operand2());
			}
		}));
		set.add(new SingleInsnFilter(new InsnPattern("imul", imm(4), reg()), new InsnTransform() {
			public Instruction apply(Instruction insn) {
				return insn.build("sal", imm(2), insn.operand2());
			}
		}));
		set.add(new SingleInsnFilter(new InsnPattern("imul", imm(8), reg()), new InsnTransform() {
			public Instruction apply(Instruction insn) {
				return insn.build("sal", imm(3), insn.operand2());
			}
		}));
		set.add(new SingleInsnFilter(new InsnPattern("imul", imm(16), reg()), new InsnTransform() {
			public Instruction apply(Instruction insn) {
	...
	class InsnPattern {
		...
		public boolean match(Instruction insn) {
			return name.equals(insn.mnemonic()) && (pattern1 == null || pattern1.match(insn.operand1()))
	...
	class JumpEliminationFilter extends Filter {
		...
		/**
		 * Returns true if jmpDest is found in asms before any instruction or
		 * directives. For example, this method returns true if contents of asms are:
		 *
		 * if_end3: # comment jmpDest: mov mov add
		 */
